...
`scheduler.max_event_cascade_depth` bounds the number of event-generated links
following an external or time-triggered dispatch. The default is eight; zero
allows external/time roots but rejects events generated by a scheduled dispatch.
Depth is execution context, not a user parameter or an event-payload field.
Incoming events and fired triggers carry it explicitly. A scheduled producer's
event increments the current depth; direct self-trigger suppression remains.

The manager rejects excessive depth before any workflow dispatcher or fallback
executor starts, and persists a skipped execution with the attempted depth and
reason. Accepted dispatches retain their depth in request evidence and scope it
over the workflow future. Service health-monitor tasks capture and restore both
schedule identity and depth across spawning and final assessment. Two schedules
that emit matching events therefore cannot reset each other's budget. Time and
operator events start at zero. Recovery of time occurrences starts a new root;
event triggers are not reconstructed from old payloads on restart.

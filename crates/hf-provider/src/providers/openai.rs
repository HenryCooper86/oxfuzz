...
impl OpenAiProvider {
    ...
    fn latest_user_prompt(request: &ChatRequest) -> Result<String, ProviderError> {
        request
            ...
            .iter()
            .rev()
            .find(|message| message.role == hf_core::types::Role::User)
            ...
            .map(|m| {
                ...
                let (content, tool_calls) =
                    if m.role == hf_core::types::Role::Assistant && !m.tool_calls.is_empty() {
                        ...
                        (content, Some(tcs))
                    } else if m.role == hf_core::types::Role::User {
    ...
    fn build_request_body(&self, request: &ChatRequest, stream: bool) -> OpenAiRequest {
        ...
        OpenAiRequest {
            ...
            response_format: request.response_format.as_ref().map(|rf| {
                use hf_core::provider::ResponseFormat;
                match rf {
...
/// Map the unified [`ThinkingEffort`](hf_core::provider::ThinkingEffort) to the
/// `OpenAI` wire string. `Max` is not an `OpenAI` reasoning level and is
/// downgraded to `"high"`.
fn thinking_effort_str(effort: hf_core::provider::ThinkingEffort) -> String {
    use hf_core::provider::ThinkingEffort;
    match effort {
...
mod tests {
    ...
    fn streaming_request_omits_stream_options_by_default() {
        ...
        let request = ChatRequest {
            ...
            top_p: None,
            tools: vec![],
            tool_calling_mode: hf_core::provider::ToolCallingMode::Native,
            stop: vec![],
    ...
    fn reasoning_request(effort: Option<hf_core::provider::ThinkingEffort>) -> ChatRequest {
        ChatRequest {
            messages: vec![hf_core::types::Message {
                message_id: "m1".into(),
                role: hf_core::types::Role::User,
            ...
            stop: vec![],
            extra: serde_json::Value::Null,
            thinking: effort.map(|effort| hf_core::provider::ThinkingConfig { effort }),
            response_format: None,
            image_generation_options: None,
        }
    }
    ...
    fn request_body_uses_nested_reasoning_by_default() {
        ...
        let request = reasoning_request(Some(hf_core::provider::ThinkingEffort::High));
        let json = serde_json::to_value(provider.build_request_body(&request, false)).unwrap();
    ...
    fn request_body_uses_reasoning_effort_when_opted_in() {
        ...
        let request = reasoning_request(Some(hf_core::provider::ThinkingEffort::Medium));
        let json = serde_json::to_value(provider.build_request_body(&request, false)).unwrap();
    ...
    fn reasoning_effort_max_downgrades_to_high() {
        let provider = reasoning_provider(true);
        let request = reasoning_request(Some(hf_core::provider::ThinkingEffort::Max));
        let json = serde_json::to_value(provider.build_request_body(&request, false)).unwrap();

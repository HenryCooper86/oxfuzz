...
impl AnthropicProvider {
    ...
    fn extract_system(request: &ChatRequest) -> Option<Vec<AnthropicSystemContent>> {
        request
            .messages
            .iter()
            .find(|m| m.role == hf_core::types::Role::System)
            .map(|m| {
    ...
    fn build_messages(request: &ChatRequest) -> Vec<AnthropicMessage> {
        let raw: Vec<AnthropicMessage> = request
            .messages
            .iter()
            .filter(|m| m.role != hf_core::types::Role::System)
            .map(|m| {
                let role = match m.role {
                    hf_core::types::Role::User | hf_core::types::Role::Tool => "user",
                    hf_core::types::Role::Assistant => "assistant",
                    hf_core::types::Role::System => unreachable!(),
                };

                // If this is a tool result, format it as a tool_result content block.
                if m.role == hf_core::types::Role::Tool {
                ...
                if m.role == hf_core::types::Role::User {
    ...
    fn build_request_body(&self, request: &ChatRequest, stream: bool) -> AnthropicRequest {
        use hf_core::provider::{ThinkingEffort, ToolCallingMode};
...
mod tests {
    ...
    fn test_build_messages_restores_assistant_tool_use_before_tool_result() {
        use hf_core::types::{Message, Role, ToolCallRequest};
    ...
    fn test_anthropic_request_with_thinking_includes_output_config() {
        use hf_core::provider::{ThinkingConfig, ThinkingEffort};
        use hf_core::types::{Message, Role};

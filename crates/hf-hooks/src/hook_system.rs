...
impl HookSystem {
    ...
    pub async fn execute_hook_handlers(
        &self,
        hook_point: hf_core::hook::HookPoint,
    ...
    pub fn set_llm_runner(&mut self, runner: std::sync::Arc<dyn hf_core::hook::HookLlmRunner>) {
    ...
    /// If no executor exists (no handlers configured), this is a no-op.
    #[cfg(all(feature = "hook_handlers", feature = "llm_hooks"))]
    pub fn set_agent_runner(&mut self, runner: std::sync::Arc<dyn hf_core::hook::HookAgentRunner>) {
        if let Some(ref mut executor) = self.handler_executor {
...
mod tests {
    ...
    use hf_core::hook::MiddlewareResult;
    ...
    async fn test_hook_system_events() {
        ...
        system
            .publish_event(Event::Tool(hf_core::hook::ToolEvent::Executed {
        ...
        assert!(matches!(
            event.as_ref(),
            Event::Tool(hf_core::hook::ToolEvent::Executed { .. })
        ));
    }

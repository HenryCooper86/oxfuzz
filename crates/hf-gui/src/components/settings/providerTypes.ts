...
export function normalizeProvider(p: Partial<Provider>): Provider {
  return {
    ...
    api_key_configured: p.api_key_configured ?? Boolean(p.api_key),
    api_key_env_configured: p.api_key_env_configured ?? Boolean(p.api_key_env),
    base_url: p.base_url ?? null,
    headers: p.headers ?? {},
    headers_configured: p.headers_configured ?? Object.keys(p.headers ?? {}).length > 0,

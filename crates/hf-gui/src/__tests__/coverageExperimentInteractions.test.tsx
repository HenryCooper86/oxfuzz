...
it("offers a separate replay review without navigating or executing", async () => {
 await mount(); await prepare();
 expect(host.textContent).toContain("Review current-input rerun");
...
it("does not offer attachable replay for a baseline without a retained seed", async () => {
 ...
 expect(host.textContent).not.toContain("Review current-input rerun");

...
impl From<ToolRegistryError> for hf_core::tool::ToolError {
    fn from(e: ToolRegistryError) -> Self {
        match e {
            ToolRegistryError::NotFound { name } => hf_core::tool::ToolError::NotFound { name },
            ToolRegistryError::ValidationError { message } => {
                hf_core::tool::ToolError::ValidationError { message }
            }
            ToolRegistryError::ExecutionError { message } => {
                hf_core::tool::ToolError::RuntimeError {
                    name: String::new(),
                    message,
                }
            }
            other => hf_core::tool::ToolError::Other {

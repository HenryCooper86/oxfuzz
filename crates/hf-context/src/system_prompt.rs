...
use hf_core::exec::RuntimeBackend;
use hf_prompt::{
...
mod tests {
    use super::*;
    use hf_prompt::{builtin_section_store, default_template};
    ...
    async fn test_per_section_budget_truncates() {
        ...
        store.register(hf_prompt::PromptSection {
            id: "core.identity".into(),
            content_source: hf_prompt::ContentSource::Inline("x".repeat(5000)),
            token_budget: 10, // Very small budget.
            priority: 100,
            condition: Some(hf_prompt::SectionCondition::Always),
            category: hf_prompt::SectionCategory::Identity,
        });

        let template = hf_prompt::PromptTemplate {
            id: "test".into(),
            parent: None,
            sections: vec![hf_prompt::SectionRef {
    ...
    async fn test_total_budget_drops_low_priority() {
        ...
        store.register(hf_prompt::PromptSection {
            id: "first".into(),
            content_source: hf_prompt::ContentSource::Inline("A".repeat(100)),
            token_budget: 500,
            priority: 100,
            condition: Some(hf_prompt::SectionCondition::Always),
            category: hf_prompt::SectionCategory::Identity,
        ...
        store.register(hf_prompt::PromptSection {
            id: "second".into(),
            content_source: hf_prompt::ContentSource::Inline("B".repeat(100)),
            token_budget: 500,
            priority: 200,
            condition: Some(hf_prompt::SectionCondition::Always),
            category: hf_prompt::SectionCategory::Behavioral,
        });
    ...
    async fn test_all_excluded_uses_fallback() {
        ...
        store.register(hf_prompt::PromptSection {
            id: "only".into(),
            content_source: hf_prompt::ContentSource::Inline("content".into()),
            token_budget: 200,
            priority: 100,
            condition: Some(hf_prompt::SectionCondition::HasTool("specific_tool".into())),
            category: hf_prompt::SectionCategory::Identity,
        });
    ...
    async fn test_custom_prompt_respects_total_budget() {
        ...
        store.register(hf_prompt::PromptSection {
            ...
            content_source: hf_prompt::ContentSource::Inline("protocol".into()),
            token_budget: 50,
            priority: 200,
            condition: Some(hf_prompt::SectionCondition::Always),
            category: hf_prompt::SectionCategory::Behavioral,

...
use hf_core::memory::{MemoryClient, MemoryError, MemoryQuery, MemoryResult};
...
mod tests {
    ...
    use hf_core::memory::MemoryType;
    use hf_core::types::{now, MemoryId};

    fn make_memory(content: &str) -> hf_core::memory::Memory {
        let ts = now();
        hf_core::memory::Memory {
            id: MemoryId::new(),

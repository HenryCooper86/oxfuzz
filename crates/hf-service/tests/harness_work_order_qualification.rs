...
async fn qualification_step_failures_return_terminal_bounded_attempts() {
    ...
    for (runtime_mode, review_mode, status, final_transition, runtime_calls, review_calls) in cases
    {
        ...
        assert!(!message.contains("embedded-assignment-secret"));
        assert!(!message.contains("embedded-bearer-credential"));
        ...
        assert!(!durable_message.contains("embedded-secret"));
        assert!(!durable_message.contains("embedded-assignment-secret"));
        assert!(!durable_message.contains("embedded-bearer-credential"));

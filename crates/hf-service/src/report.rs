...
pub struct Labels {
    ...
    /// Heading for the deterministic CASR exploitability classifications.
    pub casr_verdicts_section: &'static str,
    /// Explanation that CASR exploitability is evidence, not proof that the
    /// target contains a vulnerability.
    pub casr_verdicts_note: &'static str,
    /// Empty state when there are no findings for CASR to classify.
    pub no_casr_verdicts: &'static str,
    ...
    pub bullet_casr_verdict: &'static str,
    ...
    /// Value used when CASR produced no report for a finding.
    pub casr_unavailable: &'static str,
    // Footer
...
impl Labels {
    ...
    pub const fn english() -> Self {
        Self {
            ...
            casr_verdicts_section: "CASR Vulnerability Verdicts",
            casr_verdicts_note: "CASR classifies crash exploitability. This verdict does not \
                                 independently establish that the crash is a vulnerability in \
                                 the target.",
            no_casr_verdicts: "No target crashes were available for a CASR vulnerability verdict.",
            ...
            bullet_input: "Input",
            bullet_casr_verdict: "CASR vulnerability verdict",
...
impl Labels {
    ...
    pub const fn chinese() -> Self {
        Self {
            ...
            finding_prefix: "发现项",
            casr_verdicts_section: "CASR 漏洞判定 (CASR Vulnerability Verdicts)",
            casr_verdicts_note:
                "CASR 对崩溃的可利用性进行分类。该判定本身不能独立证明目标中存在漏洞。",
            no_casr_verdicts: "没有可供 CASR 进行漏洞判定的目标崩溃。",
            ...
            casr_unavailable: "不可用 (Unavailable)",
...
pub fn render_markdown(data: &ReportData, labels: &Labels) -> String {
    ...
    render_casr_verdicts(&mut md, data, labels);
...
/// Render the deterministic CASR evidence block included in every campaign
/// report and restored after provider composition when necessary.
fn render_casr_verdicts(md: &mut String, data: &ReportData, labels: &Labels) {
    let _ = writeln!(md, "## {}", labels.casr_verdicts_section);
    let _ = writeln!(md);
    let _ = writeln!(md, "{}", labels.casr_verdicts_note);
    let _ = writeln!(md);
    if data.crashes.is_empty() {
        let _ = writeln!(md, "{}", labels.no_casr_verdicts);
        let _ = writeln!(md);
        return;
    }

    for (index, crash) in data.crashes.iter().enumerate() {
        let _ = writeln!(
            md,
            "- {} {}{}{}",
            labels.finding_prefix,
        ...
        match &crash.casr {
            ...
            None => {
                let _ = writeln!(
                    md,
                    "  - {}{}**{}**",
                    labels.bullet_casr_verdict, labels.bullet_colon, labels.casr_unavailable
                );
            }
        }
    }
...
pub fn report_user_prompt(facts: &str, data: &ReportData, language: ReportLanguage) -> String {
    ...
    format!(
        "Compose a comprehensive, professional fuzzing report in GitHub-flavored \
         ...
         Preserve every ```mermaid``` code block, Markdown table, and the complete \
         deterministic CASR vulnerability-verdict section from the data sheet \
         verbatim, placing them in the most relevant section.\n\n\
...
/// Guarantee the report carries the deterministic campaign evidence.
///
/// If the model drops the Mermaid blocks or changes the CASR verdict section,
/// append the corresponding deterministic rendering. Also stamps the generator
/// footer.
///
...
#[must_use]
pub fn ensure_required_evidence(ai_markdown: &str, data: &ReportData, labels: &Labels) -> String {
    ...
    let mut casr_verdicts = String::new();
    render_casr_verdicts(&mut casr_verdicts, data, labels);
    if !out.contains(casr_verdicts.trim()) {
        out.push_str("\n\n");
        out.push_str(casr_verdicts.trim_end());
        out.push('\n');
    }
    let _ = write!(
        out,
